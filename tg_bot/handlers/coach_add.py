...
@router.message(StateFilter(FSMAdd.add_choose_type), MaterialType(types=[1, 2]))
async def add_choose_type(message: Message, state: FSMContext, cell: int):
    ...
    msg = await message.answer(text='Пришлите анимацию в формате gif, которая демонстрирует выполнение упражнения. '
                               ...
                               reply_markup=ReplyKeyboardMarkup(keyboard=[[
                                   KeyboardButton(text="Пропустить (только если ОБНОВЛЯЕТЕ материал)")]],
                                   one_time_keyboard=True, resize_keyboard=True))
    data['delete_list'].append(msg.message_id)
    await state.update_data(delete_list=data['delete_list'])
    logger.debug(f'{cell=}')
    await state.update_data(type=cell)
    await state.set_state(FSMAdd.type_exercise_1)


@router.message(StateFilter(FSMAdd.add_choose_type), MaterialType(types=[3, 4]))
async def add_choose_type(message: Message, state: FSMContext, cell: int):
    data = await state.get_data()
    data['delete_list'].append(message.message_id)
    if message.text.strip().isdigit():
...
@router.message(StateFilter(FSMAdd.add_choose_type), MaterialType(types=[5, 6, 7]))
async def add_choose_type(message: Message, state: FSMContext, cell: int):
    data = await state.get_data()
...
@router.message(StateFilter(FSMAdd.add_choose_type), MaterialType(types=[8]))
...
@router.message(StateFilter(FSMAdd.add_choose_type))
async def add_choose_type(message: Message, state: FSMContext):
    logger.debug('enter error add_choose_type')
    data = await state.get_data()
    msg = await message.answer(text='При обновлении материалов произошла ошибка, начать заново?', reply_markup=yesno)
    data['delete_list'].append(message.message_id)
    data['delete_list'].append(msg.message_id)
    await state.update_data(delete_list=data['delete_list'])
...
@router.message(F.text.strip().lower().startswith('пропустить'), StateFilter(FSMAdd.type_exercise_1))
async def type_exercise_1(message: Message, state: FSMContext, db: SQLiteDatabase):
    data = await state.get_data()
    logger.debug(f'enter type_exercise_1 {data=}')

    if 'exercise_id' in data:
        material = db.select_rows(table='materials', fetch='one',
                                  user_id=message.from_user.id, exercise_id=data['exercise_id'])
        if not material:
    ...
    elif 'file_unique_id' in data:
        material = db.select_rows(table='materials', fetch='one',
                                  user_id=message.from_user.id, exercise_id=data['file_unique_id'])
        if not material:
            db.add_material(user_id=message.from_user.id, type_=data['type'], exercise_id=data['file_unique_id'],
                            date=datetime.utcnow().isoformat())
        msg = await message.answer(text='Пришлите ссылку на публичное видео с русской озвучкой, '
                                        'где подробно объясняется и показывается, как выполняется данное упражнение, '
                                        'какие могут быть нюансы, ошибки и противопоказания:',
...
@router.message(F.photo, StateFilter(FSMAdd.type_exercise_1))
...
@router.message(F.animation, StateFilter(FSMAdd.type_exercise_1))
...
async def type_exercise_2(message: Message, state: FSMContext, db: SQLiteDatabase):
    data = await state.get_data()
    data['delete_list'].append(message.message_id)
    if message.text.strip().lower().startswith('пропустить'):
...
async def type_exercise_4(message: Message, state: FSMContext, db: SQLiteDatabase):
    data = await state.get_data()
    data['delete_list'].append(message.message_id)
    if (message.text.strip().lower() != 'нет') and (not message.text.strip().lower().startswith('пропустить')):
        if 'file_unique_id' in data:
...
async def add_work_2(message: Message, state: FSMContext, db: SQLiteDatabase, bot: Bot, config: Config):
    data = await state.get_data()
    data['delete_list'].append(message.message_id)
    cells = {'arms': data['work']['arms'] / 100, 'legs': data['work']['legs'] / 100, 'chest': data['work']['chest'] / 100,
             'abs': data['work']['abs'] / 100, 'back': data['work']['back'] / 100}
    if 'file_unique_id' in data:
        db.update_cells(table='materials', cells=cells, file_unique_id=data['file_unique_id'])
    elif 'exercise_id' in data:
        db.update_cells(table='materials', cells=cells, exercise_id=data['exercise_id'])
