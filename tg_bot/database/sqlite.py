...
class SQLiteDatabase:
    ...
    def add_workout_short(self, workout_id: int, user_id: int, date: str, approaches: int,
                          work: float, arms: float, legs: float, chest: float, abs: float, back: float, new=True):
        sql = ('INSERT INTO workouts_short (workout_id, user_id, date, approaches, work, arms, legs, chest, abs, back) '
               'VALUES(?,?,?,?,?,?,?,?,?,?)')
        parameters = (workout_id, user_id, date, approaches, work, arms, legs, chest, abs, back)
        self.execute(sql, parameters=parameters, commit=True, new=new)
    ...
    def clear_table(self, table, new=False):
        self.execute(f'DELETE FROM {table} WHERE True', commit=True, new=new)

    def delete_table(self, table, new=False):
        self.execute(f'DROP TABLE IF EXISTS {table};', commit=True, new=new)
